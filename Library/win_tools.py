...
def select_options(ph_sumbit, ph_quit):
	...
	b1 = Button(win, image=ph_sumbit,
				command=(
					lambda r=radioVar, w=ent_from, t=ent_to, s=ent_key,
						   l=[v1, v2, v3, v4], p=ph_quit: select_result(r, w, t, s, l, p)))
...
# До этого момента все норм работает теперь, то есть выводится список в консоль
# осталось его через шоу тейбл просто отобразить, что я и делаю)))
...
def stats_win(ph_save, ph_quit):
	...
	l1 = Label(win, text=w.disp + str(dh.get_dispersion(dh.db_path)), font=(w.font_ab, 10), bg=w.bg_col)
	l1.pack(padx=10, pady=10)
